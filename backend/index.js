...
socketConnection.on('connection',(socket)=>{
    ...
    socket.on('createGroup',(data)=>{
        console.log("------------->",data)
        chatGroup.unshift({id:chatGroup?.length+1,groupName:data.groupName,messages:[]})
    ...
    socket.on('sendMessage',(data)=>{
        ...
        if (group) {    
            ...
            group[0].messages.unshift({mesage:data.message,sender:data.sender,time:data.timeData})
            socket.emit('groupsList',chatGroup)

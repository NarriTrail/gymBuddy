...
const ChatSpecificScreen = () => {
  ...
  const {item: paramItem}: any = route?.params;
  const sendMessage = () => {
    const timeData = {
      hrs:
    ...
    const newMessage = {
      message,
      timeData,
      groupId: paramItem?.id,
      groupName: paramItem?.groupName,
  ...
  useEffect(() => {
    socket.emit('findGroup', paramItem?.id);
  ...
  return (
    <KeyboardAvoidingView
      ...
      <Header title={paramItem?.groupName} />
      <FlatList
        data={messages}

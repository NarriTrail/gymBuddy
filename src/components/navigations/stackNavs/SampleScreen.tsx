...
const TimerApp = () => {
    ...
    const interval = setInterval(() => {
      setTimers((prevTimers: any) =>
        prevTimers.map((category: {timers: []}) => ({
          ...category,
          timers: category.timers.map(
            (timer: {status: string; remaining: number}) => {
  ...
  const addTimer = () => {
    ...
    if (
      !timers.some((each: {category: string}) => each.category === category)
    ) {
  ...
  const updateTimerStatus = (id: number, action: string) => {
    const updatedTimers = timers.map((eachCategory: {timers: []}) => ({
      ...eachCategory,
      timers: eachCategory.timers.map(
        (timer: {id: number; duration: number; remaining: number}) =>
          timer.id === id
            ? {
                ...timer,
                status:
                  action === 'start'
                    ...
                    : 'Paused',
                remaining:
                  action === 'reset' ? timer.duration : timer.remaining,
              }
            : timer,
      ),
    }));
  ...
  const bulkAction = (cat: string, action: string) => {
    const updatedTimers = timers.map(
      (eachCategory: {category: string; timers: []}) =>
  ...
  const renderTimer = ({
    ...
    <View style={styles.timerCard}>
      ...
      {item.status !== 'Completed' && (
        <Text style={styles.timeLeftText}>Time Left: {item.remaining}s</Text>
      )}
      <ProgressBar
      ...
      <View style={styles.timerButtons}>
        <Button onPress={() => updateTimerStatus(item.id, 'start')}>
          Start
        </Button>
        <Button onPress={() => updateTimerStatus(item.id, 'pause')}>
          Pause
        </Button>
        <Button onPress={() => updateTimerStatus(item.id, 'reset')}>
          Reset
        </Button>
      </View>
    </View>
  ...
  const renderCategory = ({item}: {item: {category: string; timers: []}}) => (
    <View style={styles.categoryContainer}>
      <TouchableOpacity
        ...
        <AntDesign
          name={expandedCategories[item.category] ? 'up' : 'down'}
          size={20}
          color={'#aaa'}
        />
      </TouchableOpacity>
      {expandedCategories[item.category] && (
        <>
          <View style={styles.bulkButtons}>
            <TouchableOpacity
              style={[styles.categoryButtons, {backgroundColor: '#6AB46D'}]}
              onPress={() => bulkAction(item.category, 'Running')}>
              <Text style={styles.categotyBtnText}>Start All</Text>
            </TouchableOpacity>
            <TouchableOpacity
              style={[
                styles.categoryButtons,
                {backgroundColor: 'rgba(123, 119, 119, 0.3)'},
              ]}
              onPress={() => bulkAction(item.category, 'Paused')}>
              <Text style={styles.categotyBtnText}>Pause All</Text>
            </TouchableOpacity>
            <TouchableOpacity
              style={[
                styles.categoryButtons,
                {backgroundColor: 'rgba(227, 15, 15, 0.4)'},
              ]}
              onPress={() => bulkAction(item.category, 'reset')}>
              <Text style={styles.categotyBtnText}>Rest All</Text>
            </TouchableOpacity>
          </View>

...
console.log("isDeviceLocationEnabled",isDeviceLocationEnabled,"isAppLocationEnabled",isAppLocationEnabled,"from hook")
...
console.log("permissionStatus----------------------------------------------------------->from hook",permissionStatus,"isGPSOn",isGPSOn)
      dispatch(setLocationStatus({ appLocationStatus: permissionStatus==='denied'?false:true, deviceLocationStatus: isGPSOn }));
    } catch (error) {
      console.error('Error checking location permissions:', JSON.stringify(error,null,2));
      dispatch(setLocationStatus({ appLocationStatus: false, deviceLocationStatus: false }));
    }
  }, [dispatch]);
